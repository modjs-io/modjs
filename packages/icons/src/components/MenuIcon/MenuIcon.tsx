...
const ModMenuIcon = styled.svg`
  fill: ${props => props.fill ? props.fill : ({theme}) => theme.color.primary};
  height: ${props => props.height ? props.height : ({theme}) => theme.icons.height.md};
  width: ${props => props.width ? props.width : ({theme}) => theme.icons.width.md};
`;

const MenuIcon = ({ fill, height, width, ...props }: IconProps) => {
  return (
    <ModMenuIcon
      {...props}
      version='1.0'
